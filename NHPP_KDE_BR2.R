...
paramo.BR2 <-al %>% select(Level_2, Level_2_ids) %>% distinct(Level_2, Level_2_ids, .keep_all = FALSE)
focal.BR <- paramo.BR2
# # objects to store data
# episodic.tibbles <- list(BR1=NA, BR2=NA, EP=NA) # store tibble data for each BR
# episodic.EST <- list(BR1=NA, BR2=NA, EP=NA) # # store episodic estimates for each BR
...
names(Trees.focal.BRs) <- focal.BR$Level_2
...
#saveRDS(Trees.focal.BRs, 'Merged_trees_for_all_BR2.rds')


##  ............................................................................
##  KDE                                                                    ####
setwd("~/Documents/My_papers/onto_phylo/data/working/RevBayes/Run1_all/stm_R500_BR/BR2")
#Trees.focal.BRs <- readRDS('Merged_trees_for_all_BR2.rds')
lapply(Trees.focal.BRs, length)
plotSimmap(Trees.focal.BRs$head[[1]], get_rough_state_cols(Trees.focal.BRs$head[[1]]),  lwd=3, pts=F,ftype="off", ylim=c(0,100))
...
names(Tb.trees) <- focal.BR$Level_2
#saveRDS(Tb.trees, 'Tb.trees_for_all_BR2.rds')
...
#saveRDS(Path.data, 'Path.data_for_all_BR2.rds')
...
setwd("~/Documents/My_papers/onto_phylo/data/working/RevBayes/Run1_all/stm_R500_BR/BR2")
#saveRDS(Edge.KDE, 'Edge.KDE_BR2.rds')
#Edge.KDE <- read_rds('Edge.KDE_BR2.rds')
#Edge.KDE <- read_rds('Edge.KDE_BR2_7Oct19.rds')

#Trees.focal.BRs <- read_rds('Merged_trees_for_all_BR2.rds')
...
#saveRDS(Edge.KDE, 'Edge.KDE_BR2_7Oct19.rds')
...
#saveRDS(NHPP.Map, 'NHPP.Map_BR2.rds')

# Plot Simmap

plot.contMap(NHPP.Map$head$loess.lambda.mean, lwd=2, outline=F, legend=F, ftype="off", outline=F)
...
stat.f <- 1/stat.f
DD <- make_dist_M (M.pers$corr, stat.f, Edge.KDE)
...
DD <- make_dist_M (M.err$corr, stat.f, Edge.KDE)
cdmd.dist <- cmdscale(DD)
plot(cdmd.dist)
text(cdmd.dist[,1], cdmd.dist[,2], rownames(cdmd.dist), cex = 0.6)
...
gg <- ggplot(data = edge.profs$wing$loess.lambda.mean, aes(x = X-Tmax, y =  Y, group = edge.id, color=Y))+
...
gg <- ggplot(data = edge.profs$wing$Maps.mean.loess.norm, aes(x = X-Tmax, y =  Y, group = edge.id, color=Y))+
  geom_line(alpha=1, size=.5)+
  scale_color_gradientn(colours = rev(rainbow(5, start = 0, end = .7)) )+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = "transparent",colour = NA), 
  ...
  scale_x_continuous(limits = c(-299, 0), breaks=-1*c(0, my.periods$max_age[2:7])%>% round(0), 
                     labels = c(0, my.periods$max_age[2:7])%>% round(0) )+
  scale_y_continuous(limits = c(-0.001, NA))+
  coord_cartesian( expand = FALSE)
#geom_vline(xintercept = 0, color='blue')+
#geom_vline(xintercept = 281, color='blue')
gggeo_scale(gg, periods=my.periods, neg=T)
...
setwd("~/Documents/My_papers/onto_phylo/data/working/RevBayes/Run1_all/stm_R500_BR/BR2/Profiles_and_Tree_Rates")
#--- Test
...
setwd("~/Documents/My_papers/onto_phylo/data/working/RevBayes/Run1_all/stm_R500_BR/BR2/Profiles_and_Tree_Derivs")
...
setwd("~/Documents/My_papers/onto_phylo/data/working/RevBayes/Run1_all/stm_R500_BR/BR2/Edge_contribution_plots")
